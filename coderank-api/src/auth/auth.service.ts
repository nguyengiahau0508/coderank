...
export class AuthService {
  ...
  async logout(userId: string, accessToken: string, refreshToken: string) {
    const isAccessTokenResvoked = await this.tokensService.revokeToken(
      accessToken,
      TokenTypeEnum.ACCESS,
    );
    const isRefreshTokenRevoked = await this.tokensService.revokeToken(
      refreshToken,
  ...
  async refreshTokens(ipAddress: string, refreshToken: string) {
    const jwtPayload: IJwtPayload = await this.tokensService.verifyToken(
      refreshToken,
      TokenTypeEnum.REFRESH,
    );
