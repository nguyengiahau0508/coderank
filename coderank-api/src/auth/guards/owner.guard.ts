...
export class OwnerGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const ownerField =
      this.reflector.get<string>('ownerField', context.getHandler()) ||
      'userId';
    const paramId =
      this.reflector.get<string>('ownerParam', context.getHandler()) || 'id';
    ...
    const entity = await repo.findOne({
      where: { id, [ownerField]: user.userId },
    });
    if (!entity) throw new ForbiddenException('Resource not found');
    if (
      entity[ownerField] !== user.userId &&
