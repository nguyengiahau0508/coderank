...
export class SubmissionsService extends BaseService<SubmissionsEntity> {
  ...
  async submit(userId: string, problemId: string, dto: CreateSubmissionDto) {
    ...
    const submissionSaved = await this.submissionRepository.save(submissionCreated);

    const problem = await this.problemsService.findOne({ where: { id: problemId }, select: ['memoryLimitMb', 'timeLimitMs'] });
    if (!problem) {
      throw new Error('Problem not found');
    }

    const testcases = await this.testcasesService.find({ where: { problemId }, select: ['input', 'expectedOutput', 'compareType'] });
